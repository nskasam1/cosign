...
COSIGN_DB="$SCRATCH" npm run serve:prod > /tmp/pc-server.log 2>&1 &
SERVER=$!
...
let s='';process.stdin.on('data',d=>s+=d).on('end',()=>{
  try{const j=JSON.parse(s);const all=[].concat(j.results||[],j.places||[],j.shops||[]);
  console.log(all.filter(p=>p.open_now).length)}catch(e){console.log('?')}})")
echo "seeded places open right now: $OPEN  (local time $(date '+%H:%M'))"
